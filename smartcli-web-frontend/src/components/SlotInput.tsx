            ...
            {p.enumOptions.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-11 min-w-0 border-r border-slate-200 px-2 py-2 text-sm font-mono transition last:border-r-0
    ...
    <label className="block">
      ...
      <div className={numeric ? 'mt-1 flex gap-1' : undefined}>
        <input
          ...
          onChange={(e) => onChange(e.target.value)}
          placeholder={p.hint}
          className={`${numeric ? '' : 'mt-1'} min-h-11 w-full bg-white dark:bg-slate-950 border rounded px-3 py-2
