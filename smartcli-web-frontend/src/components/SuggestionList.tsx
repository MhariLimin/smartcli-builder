      ...
      {suggestions.map((s, i) => {
        ...
        return (
          <li
            key={`${s.text}-${i}`}
            onClick={() => onSelect(s)}
            className="cursor-pointer px-4 py-2 hover:bg-slate-800/70 transition flex items-start gap-3"
          >
            <span className={`text-[10px] uppercase tracking-wide font-semibold px-2 py-0.5 rounded border ${colorClass}`}>
              {s.category}
            </span>
